...
const Flashcard = ({ word, onSpeakEnglish, onSpeakIndonesian }: FlashcardProps) => {
  ...
  return (
    <div className="group relative w-full max-w-xl">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-(--border) bg-(--card) p-5 shadow-(--shadow) transition duration-300 sm:p-8">
        ...
        <div className="relative z-10 flex flex-col gap-4 text-left">
          ...
          <div className="flex items-start justify-between gap-3">
            <p className="text-2xl font-semibold leading-tight text-(--text-strong) sm:text-4xl">
              {word.english}
            </p>
            <button
              type="button"
              onClick={() => onSpeakEnglish(word.english)}
              className="inline-flex h-9 items-center gap-2 rounded-full bg-(--pill) px-3 text-xs font-semibold text-(--text-strong) transition hover:-translate-y-0.5 cursor-pointer sm:h-10 sm:px-4 sm:text-sm"
          ...
          <div className="flex items-start justify-between gap-3">
            <p className="text-lg text-(--text) sm:text-2xl">{word.indonesian}</p>
            <button
              type="button"
...
export function App() {
  ...
  return (
    <div className={`app-shell relative min-h-screen overflow-hidden ${theme === "dark" ? "theme-dark" : "theme-light"}`}>
      ...
      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-4xl flex-col items-center px-3 pb-24 pt-6 sm:px-6 sm:pb-28 sm:pt-8">
        ...
        <main className="flex w-full flex-1 flex-col items-center gap-4 sm:gap-5">
          <div className="glass-card flex w-full flex-col gap-3 rounded-2xl px-3 py-3 sm:flex-row sm:items-center sm:px-5">
            <div className="flex items-center gap-3">
              <span className="flex h-9 w-9 items-center justify-center rounded-2xl bg-(--pill) text-base sm:h-10 sm:w-10 sm:text-lg">
                <Search className="h-5 w-5 text-(--text-strong)" />
              </span>
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-(--muted)">Cari kata</p>
            ...
            <div className="flex flex-1 items-center gap-2">
              <input
                ...
                className="w-full rounded-xl border border-transparent bg-(--input) px-3 py-2 text-sm text-(--text) placeholder:text-(--muted) outline-none transition focus:border-(--accent) sm:text-base"
                autoComplete="off"
              />
              {query ? (
                <button
                  type="button"
                  onClick={() => setQuery("")}
                  className="inline-flex items-center gap-2 rounded-full bg-(--pill) px-3 py-2 text-xs font-semibold text-(--text-strong) transition hover:-translate-y-0.5 sm:text-sm cursor-pointer"
                >
                  <X className="h-4 w-4" />
                  Reset
                </button>
          ...
          <div className="flex w-full flex-wrap items-center justify-between gap-2 text-xs text-(--muted) sm:text-sm">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`rounded-full px-3 py-2 text-xs font-semibold transition cursor-pointer sm:text-sm ${sortMode === "alpha"
              ...
              <button
                ...
                className={`rounded-full px-3 py-2 text-xs font-semibold transition cursor-pointer sm:text-sm ${sortMode === "shuffle"
                  ? "bg-(--accent-soft) text-(--text-strong) shadow-md"
                  : "text-(--text) hover:bg-(--pill)"
                  }`}
              >
          ...
          <div className="flex w-full flex-1 items-center justify-center">
            {currentWord ? (
              ...
              <div className="glass-card w-full max-w-xl rounded-3xl p-6 sm:p-8 text-center">
                <p className="text-base font-semibold text-(--text-strong) sm:text-lg">No matches found</p>
      ...
      <nav className="fixed bottom-5 left-1/2 z-20 w-[min(520px,calc(100%-1.5rem))] -translate-x-1/2">
        <div className="floating-bar grid grid-cols-5 items-center gap-1 rounded-2xl px-2 py-1.5 shadow-(--shadow) backdrop-blur-xl sm:gap-1.5 sm:px-2.5 sm:py-2">
          {[
            ...
            <button
              ...
              className="group flex flex-1 flex-col items-center gap-1 rounded-2xl px-1.5 py-1.25 text-[11px] font-semibold text-(--text-strong) transition duration-200 hover:-translate-y-1 hover:bg-(--pill) cursor-pointer sm:px-2 sm:py-1.5 sm:text-xs"
            >
              <span className="flex h-8 w-8 items-center justify-center rounded-2xl bg-[color-mix(in_srgb,var(--accent) 10%,transparent)] text-(--text-strong) shadow-sm transition group-hover:shadow sm:h-9 sm:w-9">
                {item.icon}
              </span>
